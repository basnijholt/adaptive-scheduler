...
def add_timing_to_object(
    obj: Any,
    timing_dict: dict[str, list[float]] | None = None,
    *,
    ...
    Args:
        obj: The object whose methods should be timed
        timing_dict: Optional existing dictionary to store times in
    ...
    def timing_wrapper(method_name: str, original_method: Callable[..., Any]) -> Callable[..., Any]:
    ...
    for attr_name in dir(obj):
        ...
        if attr_name.startswith("__") and not include_magic:
            continue
        if attr_name.startswith("_") and not attr_name.startswith("__") and not include_private:
            continue
        ...
        try:
            ...
            if callable(attr) and not isinstance(attr, type):
                # Wrap the method and replace it
                wrapped_method = timing_wrapper(attr_name, attr)
                setattr(obj, attr_name, wrapped_method)
        except (AttributeError, TypeError):
            # Some attributes might not be accessible or wrappable
            pass
